...
import { EDITIONS, getEditionBySlug, type CAEdition } from "../_data/editions";
...
function buildSchemas(edition: CAEdition) {
  ...
  const article = {
    ...
    "@type": edition.type === "daily" ? "NewsArticle" : "Article",
    headline: edition.metaTitle,
    description: edition.metaDescription,
    datePublished: edition.publishedDate,
    dateModified: edition.publishedDate,
    ...
    mainEntityOfPage: {
      "@type": "WebPage",
      "@id": `https://getvidya.in/current-affairs/${edition.slug}`,
    },
    about: edition.aboutThings.map((name) => ({ "@type": "Thing", name })),
  };
...
export default function CAEditionPage({ params }: { params: { slug: string } }) {
  const edition = getEditionBySlug(params.slug);
  if (!edition) notFound();

  const { bc, article, faqSchema } = buildSchemas(edition);
          ...
          <div className="container-xl max-w-4xl">
            ...
            <div className="flex flex-wrap gap-2 mb-4">
              <span className="inline-flex items-center gap-1.5 bg-accent/20 border border-accent/30 text-accent text-xs font-semibold px-3 py-1.5 rounded-full">
                {isDaily ? (
                  <><span className="w-1.5 h-1.5 rounded-full bg-accent animate-pulse" /> Today&apos;s Daily CA</>
                ) : (
                  "Weekly Edition"
                )}
              </span>
            ...
            <h1 className="text-3xl md:text-4xl lg:text-5xl font-bold text-white mb-4 leading-tight">
              {isDaily ? "Current Affairs" : "Current Affairs"}<br />
              <span className="text-accent">{edition.dateRange}</span>
            </h1>
            <p className="text-white/70 mb-6 text-sm md:text-base">
              {edition.examTags} — curated by GetVidyaAI
            </p>
            <div className="flex flex-wrap items-center gap-5 text-white/50 text-sm">
              <span className="flex items-center gap-1.5">
                <Calendar size={13} />
                Published: {formatPublishedDate(edition.publishedDate)}
              </span>
              <span className="flex items-center gap-1.5">
                <Clock size={13} />
                {totalItems} key event{totalItems !== 1 ? "s" : ""} · {edition.sections.length} categor{edition.sections.length !== 1 ? "ies" : "y"}
              </span>
            </div>
          </div>
        </section>
          ...
          <section className="py-16 bg-white">
            <div className="container-xl max-w-3xl">
              <h2 className="text-2xl font-bold text-primary-500 mb-8">
                Frequently Asked — {edition.dateRange}
              </h2>
        ...
        <section className="py-16 bg-gradient-hero">
          <div className="container-xl max-w-2xl text-center">
            ...
            <p className="text-white/70 mb-6 text-sm">
              GetVidyaAI tracks your CA accuracy and adjusts your weekly practice automatically.
            </p>
            <div className="flex flex-col sm:flex-row gap-3 justify-center">
              <Link
                href="https://app.getvidya.in"
                className="btn-primary px-7 py-3.5 rounded-2xl inline-flex items-center gap-2"
              >
