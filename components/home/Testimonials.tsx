  ...
  return (
    <section className="py-24 bg-white">
      <div className="container-xl">
        ...
        <AnimateIn className="relative max-w-4xl mx-auto">
            ...
            <AnimatePresence mode="wait" custom={direction}>
              ...
              >
                ...
                <div className="flex items-center gap-4">
                  <div className="w-12 h-12 rounded-full bg-gradient-to-br from-teal to-primary-300 flex items-center justify-center text-white font-bold text-lg flex-shrink-0">
                    {testimonials[current].name[0]}
                  </div>
                  <div>
                    <div className="font-bold text-primary-500">{testimonials[current].name}</div>
                    <div className="text-sm text-teal font-medium">{testimonials[current].exam}</div>
                  </div>
                </div>
              </motion.div>
            </AnimatePresence>
            ...
            <div className="flex gap-2">
              {testimonials.map((_, i) => (
                <motion.button
                  ...
                  className={`rounded-full h-2 transition-colors ${i === current ? "bg-primary-500" : "bg-slate-300 hover:bg-teal"}`}
        ...
        {/* Mini review grid */}
        <StaggerContainer className="grid grid-cols-1 sm:grid-cols-3 gap-5 mt-16">
          {testimonials.slice(0, 3).map((t) => (
            <StaggerItem key={t.name}>
              <motion.div
                className="card p-5 border border-slate-100 h-full"
                whileHover={{ y: -4, boxShadow: "0 12px 32px rgba(10,40,36,0.10)" }}
